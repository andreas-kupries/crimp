...
crimp_input (imageAObj, imageA, double);
...
result = crimp_new_double_at (bb.x, bb.y, bb.w, bb.h);
oxa = crimp_x (imageA);
oya = crimp_y (imageA);
oxb = crimp_x (imageB);
oyb = crimp_y (imageB);
...
for (py = 0, ly = bb.y, pya = bb.y - oya, pyb = bb.y - oyb;
	...
	double a_v = ina ? DOUBLEP (imageA, pxa, pya) : BLACK;
	double b_v = inb ? DOUBLEP (imageB, pxb, pyb) : BLACK;
	
	double z_vv = BINOP (a_v, b_v);
	
	DOUBLEP (result, px, py) = BINOP_POST (z_vv);
    }
}
