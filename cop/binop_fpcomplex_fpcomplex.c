...
for (py = 0; py < bb.h; py++) {
	...
	double a_re = ina ? RE (imageA, lx - oxa, ly - oya) : BLACK;
	double a_im = ina ? IM (imageA, lx - oxa, ly - oya) : BLACK;
	double b_re = inb ? RE (imageB, lx - oxb, ly - oyb) : BLACK;
	double b_im = inb ? IM (imageB, lx - oxb, ly - oyb) : BLACK;
	
	RE (result, px, py) = BINOP (a_re, b_re);
	IM (result, px, py) = BINOP (a_im, b_im);
    }
}
